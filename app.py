...
q_price = st.number_input(
    "Entrepreneur ile üreteceğin ürünün PP başına market fiyatı",
    min_value=0.0, step=0.001, format="%.3f"
)
q_bonus = st.number_input(
    "Şirketinin bonusu %",
    min_value=0.0, step=0.01, format="%.2f"
)

z = st.number_input(
    "Energy ile PP başına maaş",
    min_value=0.0, step=0.001, format="%.3f"
)
tax = st.number_input(
    "Maaş vergisi %",
...
k_price = st.number_input(
    "Kendi şirketinde ürettiğin ürünün PP başına fiyatı",
    min_value=0.0, step=0.001, format="%.3f"
)
k_bonus = st.number_input(
    "Şirketinin bonusu %",
    min_value=0.0, step=0.01, format="%.2f"
)
